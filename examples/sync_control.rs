...
fn main() -> Result<()> {
    ...
    println!("\n--- 使能扭矩 ---");
    for &id in &servo_ids {
        let (result, _) = sms_sts.write_torque_enable(id, true);
        match result {
    ...
    for i in 0..servo_ids.len() {
        if let Err(e) = sms_sts.sync_write_pos_ex(servo_ids[i], positions1[i], speeds1[i], accs1[i])
        {
    ...
    sms_sts.group_sync_write.clear_param();
    for i in 0..servo_ids.len() {
        if let Err(e) = sms_sts.sync_write_pos_ex(servo_ids[i], positions2[i], speeds2[i], accs2[i])
        {
            eprintln!("[ID:{}] 添加参数失败: {:?}", servo_ids[i], e);
        }
    }
    ...
    sms_sts.group_sync_write.clear_param();
    for i in 0..servo_ids.len() {
        if let Err(e) = sms_sts.sync_write_pos_ex(
            servo_ids[i],
            home_positions[i],
            home_speeds[i],
            home_accs[i],
    ...
    println!("\n--- 禁用扭矩 ---");
    for &id in &servo_ids {
        let (result, _) = sms_sts.write_torque_enable(id, false);
        match result {
