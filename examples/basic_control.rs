...
fn main() -> Result<()> {
    ...
    println!("\n--- 使能扭矩 ---");
    let (result, _) = sms_sts.write_torque_enable(servo_id, true);
    match result {
        COMM::Success => println!("[ID:{}] 扭矩使能成功 ✓", servo_id),
        _ => println!("[ID:{}] 扭矩使能失败: {:?}", servo_id, result),
    }

    // 3. 写入位置 (对应Python: sms_sts.WritePosEx(servo_id, position, speed, acc))
    println!("\n--- 位置控制 ---");
    ...
    let (result, _) = sms_sts.write_pos_ex(servo_id, target_position, move_speed, acc);
    match result {
    ...
    println!("\n--- 禁用扭矩 ---");
    let (result, _) = sms_sts.write_torque_enable(servo_id, false);
    match result {
