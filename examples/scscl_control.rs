...
use ftservo_sdk::{create_port_handler, create_scscl, Result, COMM};
use std::thread;
...
fn main() -> Result<()> {
    ...
    println!("\n--- 使能扭矩 ---");
    let (result, _) = scscl.write_torque_enable(servo_id, true);
    match result {
    ...
    let (result, _) = scscl.write_pos(servo_id, target_position, move_time, move_speed);
    match result {
    ...
    for i in 0..servo_ids.len() {
        if let Err(e) = scscl.sync_write_pos(servo_ids[i], positions[i], times[i], speeds[i]) {
            eprintln!("[ID:{}] 添加参数失败: {:?}", servo_ids[i], e);
        }
    }

    let result = scscl.group_sync_write.tx_packet(&mut scscl.ph);
    match result {
    ...
    // 6. 禁用扭矩
    println!("\n--- 禁用扭矩 ---");
    let (result, _) = scscl.write_torque_enable(servo_id, false);
    match result {
